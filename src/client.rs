    ...
    pub async fn send(self) -> Result<ResponseMessage, ApiError> {
        let model = self.model.unwrap_or_else(|| {
            match self.client.client_type() {
                ...
                ClientLlm::ALI => DEFAULT_QWEN_MODEL.to_string(),
        ...
        self.client
            .send_message(&model, messages, max_tokens, temperature, &system_prompt)
            .await
    }
}
...
impl AnthropicClient {
    ...
    pub fn new_custom(endpoint: String, api_key: String) -> Self {
        let client = Client::new();
        AnthropicClient {
            endpoint,
            api_key,
...
#[async_trait::async_trait]
impl LlmClientTrait for AliAIClient {
    async fn send_message(
        &self,
        model: &str,
        mut messages: Vec<Message>,
...
impl LlmClient {
    ...
    pub fn new(client_type: ClientLlm, api_key: String) -> Self {
        let client: Box<dyn LlmClientTrait + Send + Sync> = match client_type {
            ...
            ClientLlm::ALI => Box::new(AliAIClient::new(api_key)),
        };
        LlmClient { client }
    }
