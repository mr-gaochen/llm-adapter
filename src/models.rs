...
#[derive(Serialize, Deserialize, Debug)]
pub struct AliAIResponse {
    pub request_id: String,
    pub usage: AliAIUsage,
    pub output: AliAIChoice,
}

#[derive(Serialize, Deserialize, Debug, Default)]
pub(crate) struct AliAIUsage {
    pub output_tokens: usize,
    pub total_tokens: usize,
    pub input_tokens: usize,
}
...
#[derive(Serialize, Deserialize, Debug)]
pub(crate) struct AliAIChoice {
    pub choices: Vec<AliChoiceDetails>,
}

#[derive(Serialize, Deserialize, Debug)]
pub(crate) struct AliChoiceDetails {
    pub message: OpenAIMessage,
    pub finish_reason: String,
}
...
impl ResponseMessage {
    ...
    pub fn first_message(&self) -> String {
        match self {
            ...
            ResponseMessage::AliAI(response) => {
    ...
    pub fn model(&self) -> &str {
        match self {
            ...
            ResponseMessage::AliAI(response) => &response.request_id,
        }
    }
    ...
    pub fn usage(&self) -> CommonUsage {
        match self {
            ...
            ResponseMessage::AliAI(response) => CommonUsage {
                input_tokens: response.usage.input_tokens,
                output_tokens: response.usage.output_tokens,
            },
        }
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            ResponseMessage::AliAI(response) => {
                write!(
                    f,
                    "ResponseMessage {{ request_id: {:?}, usage: {:?}, choices: {:?} }}",
                    response.request_id, response.usage, response.output.choices
                )
            }
        }
    }
